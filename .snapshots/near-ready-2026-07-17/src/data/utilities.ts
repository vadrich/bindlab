...
export const UTILITY_META: Record<UtilityId, UtilityMeta> = {
  ...
  mouse: {
    ...
    kind: 'mixed',
    hint: 'Sensitivity, raw input, viewmodel и бинды смены сенсы',
  },
  inspect: {
    id: 'inspect',
    label: 'Осмотр и дроп',
    ...
    kind: 'bind',
    hint: 'Осмотр оружия, drop и смена руки',
  },
  chat: {
    id: 'chat',
    label: 'Чат',
...
function linesForUtility(
  ...
  switch (id) {
    ...
    case 'inspect':
      return { settings: [], binds: buildInspectBindLines(config.inspect) }
    case 'quick':
