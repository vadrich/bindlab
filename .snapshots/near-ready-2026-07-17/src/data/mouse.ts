...
export function buildMouseSettingsLines(cfg: MouseConfig): string[] {
  ...
  if (cfg.includeViewmodelFov) {
    lines.push(`viewmodel_fov ${fmt(clamp(cfg.viewmodelFov, 54, 68), 1)}`)
  }
  if (cfg.includeViewmodelOffsets) {
    lines.push(
      `viewmodel_offset_x ${fmt(clamp(cfg.viewmodelOffsetX, -2, 2.5))}`,
    )
    lines.push(
      `viewmodel_offset_y ${fmt(clamp(cfg.viewmodelOffsetY, -2, 2))}`,
    )
    lines.push(
      `viewmodel_offset_z ${fmt(clamp(cfg.viewmodelOffsetZ, -2, 2))}`,
    )
  }
  return lines
}
