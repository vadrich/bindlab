...
export function BindKeyCapture({
  ...
  const inputClass = [
    'min-w-0 flex-1 rounded border font-mono outline-none transition-colors',
    compact ? 'px-2 py-1.5 text-xs' : 'px-3 py-2 text-sm',
    listening
      ? 'border-[var(--accent)] bg-[var(--accent-soft)] text-transparent caret-transparent'
      : 'border-[#2a3340] bg-[#0d1117] text-white focus:border-[var(--accent)]',
    className,
  ]
    .filter(Boolean)
    .join(' ')
  ...
  const btnClass = [
    'shrink-0 rounded border font-semibold uppercase tracking-wide transition-colors',
    compact ? 'px-2 py-1.5 text-[10px]' : 'px-3 py-2 text-xs',
    listening
      ? 'border-[var(--accent)] bg-[var(--accent)] text-[var(--accent-on)]'
      : 'border-[#2a3340] text-[#9ca3af] hover:border-[var(--accent)]/50 hover:text-[var(--accent)]',
  ].join(' ')
