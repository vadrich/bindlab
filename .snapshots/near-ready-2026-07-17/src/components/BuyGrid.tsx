...
export function BuyGrid({
  ...
  return (
    <section
      className={[
        'relative flex min-w-0 flex-1 flex-col rounded-lg border border-[#2a3340] bg-[#121820]',
        matchSidebarHeight ? 'h-full min-h-0 overflow-hidden' : '',
      ].join(' ')}
    >
      <div className="relative flex shrink-0 items-center justify-between border-b border-[#2a3340] px-4 py-3">
        {showGuide || homeHint || isIdle ? (
          ...
          <button
            ...
            className="flex items-center gap-2 rounded border border-[#2a3340] bg-[#0d1117] px-2.5 py-1.5 text-xs text-[#9ca3af] transition-colors hover:border-[var(--accent)]/50 hover:text-[var(--accent-muted)]"
            title={m.common.back}
          >
      ...
      {showGuide && onDismissGuide ? (
        ...
        <div
          className={[
            'relative flex min-h-0 flex-1 flex-col overflow-hidden rounded-b-lg border border-transparent',
        ...
        <div
          className={[
            'rounded-b-lg border border-transparent p-4',
      ...
      {isWeapons && !showGuide ? (
        <p className="shrink-0 border-t border-[#2a3340] px-4 py-2 text-center text-[10px] uppercase tracking-widest text-[#4b5563]">
          {m.buy.footer}
        </p>
      ) : null}
    </section>
