...
function AudioSlider({
  ...
  /** Hide include-checkbox (always-on controls) */
  hideToggle?: boolean
}) {
...
function UtilityCard({
  ...
  return (
    <button
      ...
      className={[
        'flex flex-col overflow-hidden rounded-2xl border text-left transition-all duration-300 ease-out',
        large ? 'w-52 sm:w-56' : 'w-36 sm:w-40',
        searchSpotlightClass(spotlight),
        selected
          ? 'border-[var(--accent)]/90 bg-[var(--accent-soft-bg)] shadow-[var(--accent-glow)] -translate-y-0.5'
          : 'border-white/10 bg-black/40 shadow-[inset_0_1px_0_rgba(255,255,255,0.05)] hover:-translate-y-1 hover:border-[rgba(var(--accent-rgb),0.5)] hover:shadow-[0_12px_28px_rgba(0,0,0,0.35),0_0_20px_rgba(var(--accent-rgb),0.18)]',
      ].join(' ')}
    >
      <div
        className={[
...
function PracticeBhopBlock({
  ...
  return (
    <div
      className={[
        'relative overflow-hidden rounded-xl border p-3 transition-all duration-200 ease-out',
        on
          ? 'border-[var(--accent)]/45 bg-[var(--accent-soft)]'
          : 'border-[#2a3340] bg-[#0d1219] hover:border-[var(--accent)]/35',
      ].join(' ')}
    >
...
function ActiveSettings({
  ...
  if (activeId === 'map_cleanup') {
    return (
      <div
        ...
        className={[
          'flex min-h-[280px] flex-col items-center justify-center rounded-2xl border border-white/10 bg-black/35 px-6 py-10 text-center shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]',
          searchSpotlightClass(spotlight),
        ].join(' ')}
      >
        <div
  ...
  return (
    <div
      id={spotlight ? `search-target-${activeId}` : undefined}
      className={[
        'rounded-2xl border border-white/10 bg-black/35 p-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.04)]',
        searchSpotlightClass(spotlight),
      ].join(' ')}
    >
