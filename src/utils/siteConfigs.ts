...
function mergeUtilities(saved: unknown): UtilitiesConfig {
  ...
  // Migrate old mouse viewmodel fields → hands (inspect) card
  const mouse = out.mouse as Record<string, unknown> | undefined
  const inspect = out.inspect as Record<string, unknown> | undefined
  if (mouse && inspect) {
    if (mouse.includeViewmodelFov || mouse.includeViewmodelOffsets) {
      inspect.includeSettings = true
      if (typeof mouse.viewmodelFov === 'number') {
        inspect.viewmodelFov = mouse.viewmodelFov
      }
      if (typeof mouse.viewmodelOffsetX === 'number') {
        inspect.viewmodelOffsetX = mouse.viewmodelOffsetX
      }
      if (typeof mouse.viewmodelOffsetY === 'number') {
        inspect.viewmodelOffsetY = mouse.viewmodelOffsetY
      }
      if (typeof mouse.viewmodelOffsetZ === 'number') {
        inspect.viewmodelOffsetZ = mouse.viewmodelOffsetZ
      }
      mouse.includeViewmodelFov = false
      mouse.includeViewmodelOffsets = false
    }
  }
