...
export function generateStaticParams() {
  return landingSlugs('binds').map((slug) => ({ slug }))
}

export async function generateMetadata({ params }: Props) {
  const { slug } = await params
  const landing = landingsByGroup('binds').find((l) => l.slug === slug)
  if (!landing) return {}
  return metadataForLanding(landing)
}

export default async function BindLandingPage({ params }: Props) {
  const { slug } = await params
  const landing = landingsByGroup('binds').find((l) => l.slug === slug)
  if (!landing) notFound()
